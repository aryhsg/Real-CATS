...
這個檔案在做的事情：
1. 讀取合併後的比特幣資料集 (eth_data.tsv)
2. 淘汰不須放入模型之欄位
3. 檢查並填補 NaN 值
4. 針對分布較極端的欄位，進行對數轉換 以減少偏態
5. 計算欄位之間的相關性矩陣，並輸出至CSV檔案
6. 繪製相關性熱圖
7. 儲存處理後的數據到新的TSV檔案(eth_data_features.tsv)

"""

eth_data = pd.read_csv("eth_data_processed1.tsv", sep="\t")


# 提取標籤欄位，並儲存為y_train.tsv
y_train = eth_data["label_idx"].values
...
columns_to_use = eth_data.drop(columns=drop_cols)
print(f"原始欄位數: {len(eth_data.columns)}")
...
# 儲存處理後的數據
columns_to_use.to_csv("eth_data_features2.tsv", index=False, sep="\t")